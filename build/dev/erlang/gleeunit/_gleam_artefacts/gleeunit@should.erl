...
-spec be_ok({ok, EER} | {error, any()}) -> EER.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, EEW}) -> EEW.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(EEZ)) -> EEZ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;

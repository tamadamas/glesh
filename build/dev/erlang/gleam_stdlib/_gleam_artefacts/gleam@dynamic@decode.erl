...
-opaque decoder(BXW) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BXW, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BYE)) -> {ok, BYE} |
...
-spec map(decoder(CCC), fun((CCC) -> CCE)) -> decoder(CCE).
...
-spec then(decoder(CCO), fun((CCO) -> decoder(CCQ))) -> decoder(CCQ).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CCT), list(decoder(CCT))) -> decoder(CCT).
...
-spec recursive(fun(() -> decoder(CDJ))) -> decoder(CDJ).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(CBY)) -> decoder(gleam@option:option(CBY)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BZZ} | {error, BZZ})
) -> {BZZ, list(decode_error())}.
...
-spec collapse_errors(decoder(CCL), binary()) -> decoder(CCL).
...
-spec failure(CDD, binary()) -> decoder(CDD).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CDF} | {error, CDF})
) -> decoder(CDF).
...
-spec fold_dict(
    {gleam@dict:dict(CBK, CBL), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CBK, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {CBL, list(decode_error())})
) -> {gleam@dict:dict(CBK, CBL), list(decode_error())}.
...
-spec dict(decoder(CBD), decoder(CBF)) -> decoder(gleam@dict:dict(CBD, CBF)).
dict(Key, Value) ->
...
-spec list(decoder(CAR)) -> decoder(list(CAR)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BZA, list(decode_error())}, list(any())) -> {BZA,
    list(decode_error())}.
...
-spec index(
    list(BYO),
    list(BYO),
    fun((gleam@dynamic:dynamic_()) -> {BYR, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BYO)) -> {BYR, list(decode_error())})
) -> {BYR, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BXZ), fun((BXZ) -> decoder(BYB))) -> decoder(BYB).
...
-spec at(list(any()), decoder(BYL)) -> decoder(BYL).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BZJ), fun((BZJ) -> decoder(BZL))) -> decoder(BZL).
...
-spec optional_field(any(), BZP, decoder(BZP), fun((BZP) -> decoder(BZR))) -> decoder(BZR).
...
-spec optionally_at(list(any()), BZW, decoder(BZW)) -> decoder(BZW).

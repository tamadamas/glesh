...
-type option(GC) :: {some, GC} | none.

-file("src/gleam/option.gleam", 59).
-spec reverse_and_prepend(list(GR), list(GR)) -> list(GR).
...
-spec all_loop(list(option(GI)), list(GI)) -> option(list(GI)).
all_loop(List, Acc) ->
    case List of
        [] ->
...
-spec all(list(option(GD))) -> option(list(GD)).
all(List) ->
    all_loop(List, []).
...
-spec to_result(option(GZ), HC) -> {ok, GZ} | {error, HC}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, HF} | {error, any()}) -> option(HF).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(HM), fun(() -> HM)) -> HM.
...
-spec map(option(HO), fun((HO) -> HQ)) -> option(HQ).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(HS))) -> option(HS).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(HW), fun((HW) -> option(HY))) -> option(HY).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(IB), option(IB)) -> option(IB).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(IF), fun(() -> option(IF))) -> option(IF).
...
-spec values_loop(list(option(IN)), list(IN)) -> list(IN).
...
-spec values(list(option(IJ))) -> list(IJ).
values(Options) ->
    values_loop(Options, []).

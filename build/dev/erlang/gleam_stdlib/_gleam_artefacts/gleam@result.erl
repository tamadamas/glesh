...
-spec map({ok, CPO} | {error, CPP}, fun((CPO) -> CPS)) -> {ok, CPS} |
    {error, CPP}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CPV} | {error, CPW}, fun((CPW) -> CPZ)) -> {ok, CPV} |
    {error, CPZ}.
...
-spec flatten({ok, {ok, CQC} | {error, CQD}} | {error, CQD}) -> {ok, CQC} |
    {error, CQD}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CQK} | {error, CQL}, fun((CQK) -> {ok, CQO} | {error, CQL})) -> {ok,
        CQO} |
    {error, CQL}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CQT} | {error, CQU}, fun((CQT) -> {ok, CQX} | {error, CQU})) -> {ok,
        CQX} |
    {error, CQU}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CRC} | {error, any()}, CRC) -> CRC.
...
-spec lazy_unwrap({ok, CRG} | {error, any()}, fun(() -> CRG)) -> CRG.
...
-spec unwrap_error({ok, any()} | {error, CRL}, CRL) -> CRL.
...
-spec unwrap_both({ok, CRO} | {error, CRO}) -> CRO.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CRR} | {error, CRS}, {ok, CRR} | {error, CRS}) -> {ok, CRR} |
    {error, CRS}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CRZ} | {error, CSA}, fun(() -> {ok, CRZ} | {error, CSA})) -> {ok,
        CRZ} |
    {error, CSA}.
...
-spec all(list({ok, CSH} | {error, CSI})) -> {ok, list(CSH)} | {error, CSI}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 353).
-spec partition_loop(list({ok, CSW} | {error, CSX}), list(CSW), list(CSX)) -> {list(CSW),
    list(CSX)}.
...
-spec partition(list({ok, CSP} | {error, CSQ})) -> {list(CSP), list(CSQ)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CTF}, CTI) -> {ok, CTI} | {error, CTF}.
...
-spec replace_error({ok, CTL} | {error, any()}, CTP) -> {ok, CTL} | {error, CTP}.
...
-spec values(list({ok, CTS} | {error, any()})) -> list(CTS).
values(Results) ->

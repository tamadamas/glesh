...
-type dict(LC, LD) :: any() | {gleam_phantom, LC, LD}.
...
-spec to_list(dict(LM, LN)) -> list({LM, LN}).
to_list(Dict) ->
    maps:to_list(Dict).
...
-spec get(dict(MP, MQ), MP) -> {ok, MQ} | {error, nil}.
get(From, Get) ->
...
-spec has_key(dict(MD, any()), MD) -> boolean().
has_key(Dict, Key) ->
...
-spec from_list_loop(list({LW, LX}), dict(LW, LX)) -> dict(LW, LX).
...
-spec from_list(list({LR, LS})) -> dict(LR, LS).
from_list(List) ->
    maps:from_list(List).

-file("src/gleam/dict.gleam", 223).
-spec reverse_and_concat(list(OF), list(OF)) -> list(OF).
...
-spec do_keys_loop(list({OA, any()}), list(OA)) -> list(OA).
...
-spec do_values_loop(list({any(), OP}), list(OP)) -> list(OP).
...
-spec do_take_loop(dict(PT, PU), list(PT), dict(PT, PU)) -> dict(PT, PU).
...
-spec merge(dict(QC, QD), dict(QC, QD)) -> dict(QC, QD).
...
-spec upsert(dict(RQ, RR), RQ, fun((gleam@option:option(RR)) -> RR)) -> dict(RQ, RR).
upsert(Dict, Key, Fun) ->
...
-spec fold_loop(list({SC, SD}), SF, fun((SF, SC, SD) -> SF)) -> SF.
...
-spec fold(dict(RX, RY), SB, fun((SB, RX, RY) -> SB)) -> SB.
...
-spec filter(dict(OT, OU), fun((OT, OU) -> boolean())) -> dict(OT, OU).

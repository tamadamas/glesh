...
-opaque set(CYX) :: {set, gleam@dict:dict(CYX, list(nil))}.
...
-spec contains(set(CZI), CZI) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CZK), CZK) -> set(CZK).
delete(Set, Member) ->
...
-spec fold(set(CZT), CZV, fun((CZV, CZT) -> CZV)) -> CZV.
...
-spec filter(set(CZW), fun((CZW) -> boolean())) -> set(CZW).
...
-spec drop(set(DAD), list(DAD)) -> set(DAD).
drop(Set, Disallowed) ->
...
-spec take(set(DAH), list(DAH)) -> set(DAH).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(DAP), set(DAP)) -> {set(DAP), set(DAP)}.
order(First, Second) ->
...
-spec intersection(set(DAU), set(DAU)) -> set(DAU).
...
-spec difference(set(DAY), set(DAY)) -> set(DAY).
...
-spec is_subset(set(DBC), set(DBC)) -> boolean().
...
-spec is_disjoint(set(DBF), set(DBF)) -> boolean().
...
-spec each(set(DBM), fun((DBM) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CZF), CZF) -> set(CZF).
insert(Set, Member) ->
...
-spec from_list(list(CZQ)) -> set(CZQ).
from_list(Members) ->
...
-spec map(set(CZZ), fun((CZZ) -> DAB)) -> set(DAB).
map(Set, Fun) ->
...
-spec union(set(DAL), set(DAL)) -> set(DAL).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(DBI), set(DBI)) -> set(DBI).

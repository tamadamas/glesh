...
-opaque subject(DPT) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DPT)}.

-type name(DPU) :: any() | {gleam_phantom, DPU}.

-type do_not_leak() :: any().

-type selector(DPV) :: any() | {gleam_phantom, DPV}.
...
-spec 'receive'(subject(DQT), integer()) -> {ok, DQT} | {error, nil}.
...
-spec receive_forever(subject(DRB)) -> DRB.
...
-spec selector_receive(selector(DRF), integer()) -> {ok, DRF} | {error, nil}.
...
-spec selector_receive_forever(selector(DRJ)) -> DRJ.
...
-spec map_selector(selector(DRL), fun((DRL) -> DRN)) -> selector(DRN).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRP), selector(DRP)) -> selector(DRP).
merge_selector(A, B) ->
...
-spec select_map(selector(DSA), subject(DSC), fun((DSC) -> DSA)) -> selector(DSA).
...
-spec select(selector(DRW), subject(DRW)) -> selector(DRW).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSK)
) -> selector(DSK).
...
-spec select_other(selector(DSO), fun((gleam@dynamic:dynamic_()) -> DSO)) -> selector(DSO).
...
-spec deselect(selector(DSF), subject(any())) -> selector(DSF).
...
-spec select_specific_monitor(selector(DTA), monitor(), fun((down()) -> DTA)) -> selector(DTA).
...
-spec select_monitors(selector(DTD), fun((down()) -> DTD)) -> selector(DTD).
...
-spec select_trapped_exits(selector(DRT), fun((exit_message()) -> DRT)) -> selector(DRT).
...
-spec deselect_specific_monitor(selector(DTG), monitor()) -> selector(DTG).
...
-spec send_after(subject(DUC), integer(), DUC) -> timer().
...
-spec perform_call(
    subject(DTJ),
    fun((subject(DTL)) -> DTJ),
    fun((selector(DTL)) -> {ok, DTL} | {error, nil})
) -> DTL.
...
-spec call(subject(DTQ), integer(), fun((subject(DTS)) -> DTQ)) -> DTS.
...
-spec call_forever(subject(DTU), fun((subject(DTW)) -> DTU)) -> DTW.
